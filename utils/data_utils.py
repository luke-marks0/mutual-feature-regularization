...
def generate_synthetic_data(config, true_features=None, device=None):
    ...
    num_active_features_per_point = config.get('num_active_features_per_point', 36)
    ...
    if true_features is None:
        true_features = torch.randn(num_features, num_true_features, device=device, dtype=torch.float32)
    else:
        true_features = true_features.to(device=device, dtype=torch.float32)
    ...
    group_probs = [torch.pow(decay_rate, torch.arange(group_size, device=device, dtype=torch.float32)) / (1 - decay_rate) for _ in range(num_feature_groups)]
    ...
    for batch_start in tqdm(range(0, total_data_points, batch_size), desc="Generating Batches"):
        ...
        coeffs = torch.zeros(batch_size, num_true_features, device=device, dtype=torch.float32)
        ...
        for i, (group, probs) in enumerate(zip(feature_groups, group_probs)):
            ...
            if mask.any():
                ...
                coeffs[mask.nonzero(as_tuple=True)[0].unsqueeze(1), indices] = torch.rand(mask.sum(), num_active_features_per_point, device=device, dtype=torch.float32)
